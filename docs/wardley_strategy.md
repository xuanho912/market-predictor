...
这个项目不是为了自研基础设施，不是为了折腾部署，也不是为了自研通用数据平台。项目目标是打造一个能直接预测和解释大盘下一步走势的 **Market Prediction Dashboard**。
...
- 页面第一屏必须回答：今天有没有 edge，哪个指数最强，预测可信度如何，最大风险是什么，什么情况会让判断失效。
...
| Genesis / Custom | Signal Agreement | 自研重点 |
...
8. High-confidence Signal Validation
9. Daily Market Brief
...
如果某个数据源缺失，必须明确显示 `missing`、`not_available`、`fallback`、`stale_data` 或 `data_source_failed`，不能伪装成可用。
...
1. 今天有没有 edge？
2. 当前市场更像反抽、继续跌、震荡还是修复？
3. 哪个指数信号最强？
4. SPY / QQQ / IWM / DIA 的未来模拟路径是什么？
5. 历史相似情景支持还是冲突？
6. 最大风险是什么？
7. 什么情况会让预测失效？
8. 当前模型可信度是多少？
...
如果是左侧核心判断模块，并且能提高预测质量、解释质量或验证质量，才值得深挖。
