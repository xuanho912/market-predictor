...
30. Forecast Accuracy Ledger records are immutable. After a forecast is written, future runs may only backfill realized returns, best-matching scenario, primary-hit fields, and completion status. Never rewrite old forecast probabilities, scenarios, data versions, supporting evidence, or model versions.
